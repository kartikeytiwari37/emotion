...
def save_audio(form_audio):
    ...
    audio_path = os.path.join(app.root_path, 'static/kahona', audio_fn)
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        audio= os.path.join(app.root_path, 'static/kahona', audio_file)
        s=predict(audio)
        post = Post(title=form.title.data, content="Uploaded voice suggests:"+s, author=current_user,voice=audio_file)
        db.session.add(post)
        db.session.commit()
